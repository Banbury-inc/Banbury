...
// Type definitions for internal slide data structures
interface SlideDataBackground {
  type: 'image' | 'color'
  path?: string
  value?: string
}

interface SlideDataPosition {
  x: number
  y: number
  w: number
  h: number
...
interface SlideDataElement {
  type: string
  position?: SlideDataPosition
  style?: SlideDataStyle
  text?: string | TextRun[]
  src?: string
  items?: TextRun[]
  shape?: {
...
interface SlideData {
  background: SlideDataBackground
  elements: SlideDataElement[]
...
function validateDimensions(bodyDimensions: BodyDimensions, pres: PptxGenJSPresentation): string[] {
  const errors: string[] = []
  const widthInches = bodyDimensions.width / PX_PER_IN
  const heightInches = bodyDimensions.height / PX_PER_IN

  if (pres.presLayout) {
    const layoutWidth = (pres.presLayout as { width: number }).width / EMU_PER_IN
    const layoutHeight = (pres.presLayout as { height: number }).height / EMU_PER_IN
...
function validateTextBoxPosition(slideData: SlideData, bodyDimensions: BodyDimensions): string[] {
  ...
  for (const el of slideData.elements) {
    ...
    if (['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'list'].includes(el.type)) {
      ...
      const bottomEdge = (el.position?.y || 0) + (el.position?.h || 0)
...
async function addBackground(
  ...
  if (slideData.background.type === 'image' && slideData.background.path) {
    ...
    ;(targetSlide as any).background = { path: imagePath }
  } else if (slideData.background.type === 'color' && slideData.background.value) {
    ;(targetSlide as any).background = { color: slideData.background.value }
  }
}
...
function addElements(
  slideData: SlideData,
  targetSlide: PptxGenJSSlide,
  pres: PptxGenJSPresentation
): void {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      let imagePath = el.src?.startsWith('file://') ? el.src.replace('file://', '') : el.src
      ;(targetSlide as any).addImage({
        path: imagePath,
        x: el.position?.x,
        y: el.position?.y,
        w: el.position?.w,
      ...
      ;(targetSlide as any).addShape((pres as any).ShapeType.line, {
        x: el.x1,
        y: el.y1,
        w: (el.x2 || 0) - (el.x1 || 0),
        h: (el.y2 || 0) - (el.y1 || 0),
      ...
      const shapeOptions: any = {
        ...
        w: el.position?.w,
        h: el.position?.h,
        shape: el.shape?.rectRadius && el.shape.rectRadius > 0 ? (pres as any).ShapeType.roundRect : (pres as any).ShapeType.rect
      }
      ...
      if (el.shape?.rectRadius && el.shape.rectRadius > 0) shapeOptions.rectRadius = el.shape.rectRadius
      if (el.shape?.shadow) shapeOptions.shadow = el.shape.shadow

      ;(targetSlide as any).addText(el.text || '', shapeOptions)
      ...
      ;(targetSlide as any).addText(el.items || [], listOptions)
    } else {
      // Check if text is single-line (height suggests one line)
      const lineHeight = el.style?.lineSpacing || (el.style?.fontSize || 0) * 1.2
      const isSingleLine = (el.position?.h || 0) <= lineHeight * 1.5

      let adjustedX = el.position?.x || 0
      let adjustedW = el.position?.w || 0

      // Make single-line text 2% wider to account for underestimate
      if (isSingleLine) {
        const widthIncrease = (el.position?.w || 0) * 0.02
        const align = el.style?.align

        if (align === 'center') {
          // Center: expand both sides
          adjustedX = (el.position?.x || 0) - (widthIncrease / 2)
          adjustedW = (el.position?.w || 0) + widthIncrease
        } else if (align === 'right') {
          // Right: expand to the left
          adjustedX = (el.position?.x || 0) - widthIncrease
          adjustedW = (el.position?.w || 0) + widthIncrease
        } else {
      ...
      ;(targetSlide as any).addText(el.text, textOptions)
    }
  }
}
...
async function extractSlideData(page: Page): Promise<SlideData> {
  return await page.evaluate(() => {
    ...
    const shouldSkipBold = (fontFamily: string | null): boolean => {
    ...
    const getPositionAndSize = (el: Element, rect: DOMRect, rotation: number | null): { x: number; y: number; w: number; h: number } => {
      if (rotation === null) {
    ...
    const parseInlineFormatting = (
      ...
      runs: TextRun[] = [],
      baseTextTransform: (text: string) => string = (x) => x
    ): TextRun[] => {
        ...
        if (isText) {
          ...
          if (['SPAN', 'B', 'STRONG', 'I', 'EM', 'U'].includes((node as Element).tagName)) {
            ...
            if (computed.marginLeft && parseFloat(computed.marginLeft) > 0) {
              errors.push(`Inline element <${(node as Element).tagName.toLowerCase()}> has margin-left which is not supported in PowerPoint. Remove margin from inline elements.`)
            }
            if (computed.marginRight && parseFloat(computed.marginRight) > 0) {
              errors.push(`Inline element <${(node as Element).tagName.toLowerCase()}> has margin-right which is not supported in PowerPoint. Remove margin from inline elements.`)
            }
            if (computed.marginTop && parseFloat(computed.marginTop) > 0) {
              errors.push(`Inline element <${(node as Element).tagName.toLowerCase()}> has margin-top which is not supported in PowerPoint. Remove margin from inline elements.`)
            }
            if (computed.marginBottom && parseFloat(computed.marginBottom) > 0) {
              errors.push(`Inline element <${(node as Element).tagName.toLowerCase()}> has margin-bottom which is not supported in PowerPoint. Remove margin from inline elements.`)
            }
    ...
    let background: SlideDataBackground
    ...
    const elements: SlideDataElement[] = []
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.className && (el.className as any).includes && (el.className as any).includes('placeholder')) {
      ...
      if (isContainer) {
        ...
        const borderLines: SlideDataElement[] = []
      ...
      if (el.tagName === 'UL' || el.tagName === 'OL') {
        ...
        elements.push({
          ...
          style: {
            ...
            align: computed.textAlign === 'start' ? 'left' : (computed.textAlign as 'left' | 'center' | 'right' | 'justify'),
      ...
      const baseStyle: SlideDataStyle = {
        fontSize: pxToPoints(computed.fontSize),
        fontFace: computed.fontFamily.split(',')[0].replace(/['"]/g, '').trim(),
        color: rgbToHex(computed.color),
        align: computed.textAlign === 'start' ? 'left' : (computed.textAlign as 'left' | 'center' | 'right' | 'justify'),
      ...
      if (hasFormatting) {
        ...
        if (adjustedStyle.lineSpacing) {
          ...
          if (maxFontSize > adjustedStyle.fontSize) {
            const lineHeightMultiplier = (adjustedStyle.lineSpacing || 0) / adjustedStyle.fontSize
 ...
 *
 * @param htmlFile - Path to HTML file (absolute or relative to cwd)
 * @param pres - PptxGenJS presentation instance
 * @param options - Optional configuration
 * @returns Promise resolving to slide and placeholders
 */
export async function html2pptx(
  ...
  try {
    ...
    const launchOptions: { env?: Record<string, string>; channel?: string } = { env: { TMPDIR: tmpDir } }
