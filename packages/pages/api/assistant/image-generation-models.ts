...
import {
  FALLBACK_IMAGE_GENERATION_MODELS,
...
function isOpenAIImageModelId(modelId: string): boolean {
  const id = modelId.toLowerCase()
  return id.startsWith("dall-e") || id.startsWith("gpt-image")
}

function isGoogleImageModelId(modelId: string): boolean {
  const id = modelId.toLowerCase()
...
function mergeModels(models: MediaModelConfig[]): MediaModelConfig[] {
  ...
  for (const model of FALLBACK_IMAGE_GENERATION_MODELS) byId.set(model.id, model)
...
async function fetchOpenAIImageModels(): Promise<MediaModelConfig[]> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to list OpenAI image models: ${response.statusText}`)
  }
  ...
  return rows
    .filter((model) => model.id && isOpenAIImageModelId(model.id))
...
async function fetchGoogleImageModels(): Promise<MediaModelConfig[]> {
  const apiKey = getConfiguredGoogleKey()
  if (!apiKey) return []

  const listResult = await fetchGoogleGenerativeLanguageModels(apiKey)
  if (!listResult.ok) {
    throw new Error(`Failed to list Google image models: ${listResult.message}`)
  }

  return listResult.models
    .map((model) => {
      const id = getGoogleModelId(model)
      if (!id || !isGoogleImageModelId(id)) return null
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  for (const result of results) {
    if (result.status === "rejected") console.error("image-generation-models fetch failed", result.reason)
  }
