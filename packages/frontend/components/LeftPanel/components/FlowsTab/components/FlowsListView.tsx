...
interface FlowsListViewProps {
  ...
  onFlowRename: (flow: FlowItem) => void
  onFlowDuplicate: (flow: FlowItem) => void
  onFlowDeleted: (flow: FlowItem) => void
}
    ...
    <div className="flex-1 overflow-y-auto">
      {flows.map((flow) => {
        ...
        return (
          <FlowContextMenu
            key={flow.id}
            onOpen={() => onFlowSelect?.(flow)}
            onRename={() => onFlowRename(flow)}
            onDuplicate={() => onFlowDuplicate(flow)}
            onDelete={() => onFlowDeleted(flow)}
          >
            <div
