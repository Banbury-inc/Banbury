...
export const trackPageView = async (path: string, additionalData?: {
    pageTitle?: string;
    contentType?: string;
}) => {
    try {
        ...
        const referrerSource = extractReferrerSource(urlParams);
        const campaignId = extractCampaignId(urlParams);
...
function extractReferrerSource(urlParams: URLSearchParams): string | undefined {
...
function extractCampaignId(urlParams: URLSearchParams): string | undefined {
    // X/Twitter click ID
    if (urlParams.has('twclid')) {
